...
fn op_fn(op: &str, x: i32, y: i32) -> i32 {
    match op {
        "+" => x + y,
        "-" => x - y,
        "*" => x * y,
...
fn parse_input(fname: &str) -> Vec<Monkey> {
    ...
    mky_pat += r".*Operation: new = old (?P<op>\S+) (?P<op_arg>\d+)";
            ...
            if let Ok(ok_line) = tbd_line {
                ...
                /* TODO - reduce how nesty this is */
                    ...
                    if let Some(caps) = mky_re.captures(&line_agg) {
                        ...
                        let op_key = String::from(caps.name("op").unwrap().as_str());
                        ...
                        let mky = Monkey {
                            ...
                            op_arg: caps.name("op_arg").unwrap().as_str().parse().unwrap(),
